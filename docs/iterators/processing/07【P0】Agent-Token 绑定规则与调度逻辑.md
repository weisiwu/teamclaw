# 07【P0】Agent-Token 绑定规则与调度逻辑

## 背景

当前 Agent 的 `defaultModel` 字段硬编码在 `server/src/constants/agents.ts` 的 `AGENT_TEAM` 数组中，所有 Agent 共享环境变量中唯一的 API Key。无法实现：
...
- `server/src/services/tokenResolver.ts` — Token 调度器
- `server/src/routes/agentTokenBinding.ts` — Express 路由

## 依赖关系

- 依赖任务 05（API Token 数据模型）
- 后续任务 09（LLM 服务改造）依赖此任务
