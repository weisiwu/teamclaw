...
export const importRepo = {
    async upsert(task) {
        ...
        if (existing) {
            ...
            if (task.currentStep !== undefined) {
                ...
                vals.push(task.currentStep);
            }
            if (task.completedAt !== undefined) {
                sets.push(`completed_at = $${idx++}`);
                vals.push(task.completedAt ? new Date(task.completedAt) : null);
            }
            if (task.errorMessage !== undefined) {
                sets.push(`error_message = $${idx++}`);
                vals.push(task.errorMessage);
            }
            if (task.steps !== undefined) {
                sets.push(`steps = $${idx++}`);
                vals.push(JSON.stringify(task.steps));
            }
            if (sets.length === 0)
                return 0;
    ...
    async findById(taskId) {
        return queryOne('SELECT * FROM import_tasks WHERE task_id = $1', [taskId]);
    },
    async findByProject(projectId) {
        return query('SELECT * FROM import_tasks WHERE project_id = $1 ORDER BY started_at DESC', [projectId]);
    },
    async findPending() {
        return query("SELECT * FROM import_tasks WHERE status IN ('pending', 'processing') ORDER BY started_at ASC");
    },
    async delete(taskId) {
