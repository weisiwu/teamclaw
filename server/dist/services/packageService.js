...
function getPackageDir(versionId) {
    const dir = join(ARCHIVE_ROOT, versionId);
    return dir;
}
export function getPackageFilePath(versionId, buildNumber, format) {
    const dir = getPackageDir(versionId);
    const ext = format === 'tar.gz' ? 'tar.gz' : format;
    return join(dir, `build-${buildNumber}.${ext}`);
}
function getPackageUrl(versionId, buildNumber, format) {
    const ext = format === 'tar.gz' ? 'tar.gz' : format;
 ...
 */
export async function createPackage(versionId, buildNumber, projectPath, artifactPaths, format = 'zip') {
 ...
 */
export function getPackageInfo(versionId, buildNumber, format) {
    const fs = require('fs');
    const packagePath = getPackageFilePath(versionId, buildNumber, format);
 ...
 * Delete a package
 */
export function deletePackage(versionId, buildNumber, format) {
    const fs = require('fs');
    const packagePath = getPackageFilePath(versionId, buildNumber, format);
 ...
 */
export function listPackages(versionId) {
    const fs = require('fs');
    const packageDir = getPackageDir(versionId);
    if (!existsSync(packageDir))
        return [];
    const formats = ['zip', 'tar.gz', 'tar'];
    const results = [];
    try {
