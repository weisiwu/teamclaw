...
class MessageRouterService {
     ...
     * 匹配条件
     */
    matchConditions(msg, conditions) {
        // 渠道匹配
        if (conditions.channel && conditions.channel.length > 0) {
            if (!conditions.channel.includes(msg.channel)) {
                return false;
            }
        }
        // 角色匹配
        if (conditions.role && conditions.role.length > 0) {
            if (!conditions.role.includes(msg.role)) {
                return false;
            }
        }
        // 优先级范围
        if (conditions.priorityMin !== undefined) {
            if (msg.priority < conditions.priorityMin)
                return false;
        }
        if (conditions.priorityMax !== undefined) {
            if (msg.priority > conditions.priorityMax)
                return false;
        }
        // 内容包含关键词
        ...
        if (conditions.userIds && conditions.userIds.length > 0) {
            if (!conditions.userIds.includes(msg.userId))
                return false;
        }
        // 时间范围
     ...
     */
    getRouteStats() {
        const byTarget = { queue: 0, dlq: 0, drop: 0, agent: 0 };
