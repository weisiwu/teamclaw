...
export class CronService {
    ...
    async list() {
        return {
            list: Array.from(cronJobs.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()),
            total: cronJobs.size,
        };
    }
    /**
    ...
    scheduleJob(job) {
        if (!this.validateCronExpression(job.cron))
            return;
