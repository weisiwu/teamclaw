...
export async function analyzeCodeArchitecture(projectPath) {
...
function findLocalEntryFiles(dir) {
...
export function summarizeArchitecture(arch) {
    const lines = [];
    ...
    for (const mod of arch.modules) {
        ...
        if (mod.children && mod.children.length > 0) {
            ...
            if (mod.children.length > 3)
                lines.push(`    └─ ... (${mod.children.length - 3} more)`);
        }
    }
    return lines.join('\n');
}
