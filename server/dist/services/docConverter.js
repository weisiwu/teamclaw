 ...
 */
export async function convertDocuments(projectPath, projectId) {
    const converted = [];
...
function findFilesByExt(dir, ext, maxDepth, currentDepth = 0, results = []) {
...
export async function getConvertedDocs(projectId) {
    const docsDir = path.join(process.env.PROJECT_DATA_DIR || '/tmp/teamclaw/projects', projectId, 'docs');
 ...
 */
export function codeToHtml(filePath, content) {
...
function applyBasicHighlight(code, lang) {
    // Wrap each line
