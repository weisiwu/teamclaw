...
export class DashboardService {
    ...
    async getOverview() {
        ...
        const weekSummary = await tokenStatsService.getSummary(new Date(Date.now() - 7 * 24 * 3600 * 1000).toISOString().split('T')[0]);
        const monthSummary = await tokenStatsService.getSummary(new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString().split('T')[0]);
        // Today
