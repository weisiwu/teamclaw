...
function loadStore(file, defaultVal) {
    try {
...
function getWebhookStore() {
    if (!webhookStore)
        webhookStore = loadStore(DATA_FILE, { webhooks: [] });
    return webhookStore;
}
function getHistoryStore() {
    if (!historyStore)
        historyStore = loadStore(HISTORY_FILE, { history: [] });
    return historyStore;
}
function saveWebhookStore() {
...
export class WebhookService {
    ...
    async list() {
        const store = getWebhookStore();
        return {
            list: store.webhooks.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()),
     ...
     * 获取 Webhook 历史
     */
    async getHistory(webhookId, limit = 50) {
    ...
    async trigger(event, payload) {
        const store = getWebhookStore();
        const matching = store.webhooks.filter(w => w.status === 'active' && w.events.includes(event));
    ...
    async test(id) {
        ...
        if (idx !== -1) {
            ...
            if (success)
                store.webhooks[idx].successCount++;
            else
                store.webhooks[idx].failCount++;
            saveWebhookStore();
        }
        await auditService.log({
    ...
    async sendToWebhook(webhook, event, payload) {
        const historyStore = getHistoryStore();
        const store = getWebhookStore();
        const start = Date.now();
        let statusCode;
