 ...
 * 发送文本回复到指定通道
 */
export async function sendReply(req) {
 ...
 * 构建标准"已收到"回复文本
 */
export function buildAcknowledgmentReply(userName) {
    return `✅ 收到！您的消息已加入处理队列，main 会尽快回复您。`;
}
/**
 * 构建权限拒绝回复文本
 */
export function buildPermissionDeniedReply(userName, agent) {
    return `❌ 抱歉 ${userName}，您没有权限与 ${agent} 交互。如需帮助请联系管理员。`;
}
/**
 * 构建 Agent 执行中回复文本
 */
export function buildAgentBusyReply(userName) {
    return `⏳ 抱歉 ${userName}，main 目前正在处理其他任务，请稍后再试。`;
}
/**
 * 构建抢占通知文本
 */
export function buildPreemptNotification(preemptedUser, newUser) {
