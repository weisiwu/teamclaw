 ...
 */
export function buildDAG(taskId) {
...
// ============ 子任务树 ============
export function buildSubtaskTree(taskId, maxDepth = 5) {
    const task = taskStore.get(taskId);
    if (!task)
        return null;
...
export function detectDependencyConflicts(taskId) {
    const conflicts = [];
    const task = taskStore.get(taskId);
    if (!task)
        return conflicts;
