 ...
 */
export function detectMentionedAgent(content) {
    const patterns = [
 ...
 */
export async function processMessage(request) {
    ...
    const mentionedAgent = detectMentionedAgent(request.content) || request.mentionedAgent;
    if (mentionedAgent) {
 ...
 * 无权限 → 回复拒绝
 */
async function handleAgentMention(message, request, agent) {
    const { userId, userName, role: userRole } = request;
    // 权限校验
    const permission = checkPermission(userRole, agent);
