 ...
 */
export async function storeVersionVector(entry) {
 ...
 */
export async function searchSimilarVersions(query, limit = 5) {
    const collection = await getOrCreateCollection(COLLECTION_NAME);
    ...
    return results.ids[0].map((id, i) => ({
        ...
        versionTag: results.metadatas?.[0]?.[i]?.versionTag ?? '',
 ...
 */
export async function deleteVersionVector(versionId) {
    const collection = await getOrCreateCollection(COLLECTION_NAME);
