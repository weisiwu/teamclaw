 ...
 */
export function tryAcquireLock(resource, owner) {
 ...
 * 获取锁持有者
 */
export function getLockOwner(resource) {
    const lockPath = getLockPath(resource);
    if (!fs.existsSync(lockPath))
        return null;
...
export function listAllLocks() {
    const resources = ["skills", "workspace", "memory", "project"];
