...
async function onTaskDone(task, _oldStatus, _newStatus) {
    if (task.status === 'done') {
 ...
 */
async function onTaskFailed(task, _oldStatus, _newStatus) {
 ...
 * 任务创建时记录统计
 */
async function onTaskCreated(_task) {
    // 记录创建（统计用）
    void _task;
}
/**
 * 任务开始执行时记录
 */
async function onTaskStarted(task) {
