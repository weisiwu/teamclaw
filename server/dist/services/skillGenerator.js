...
export async function generateSkills(projectName, projectPath, arch) {
    // 确保 skills 目录存在
...
function generateBuildSkill(name, projectPath, arch) {
    const arch2 = arch;
...
function generateStructureSkill(name, arch) {
    const arch2 = arch;
...
${arch2.modules?.map((m) => `- \`${m.name}/\` — ${getModuleDescription(m.name)}`).join('\n') || '无模块信息'}
...
function generateDeploySkill(name, projectPath, arch) {
    const hasDocker = arch.modules?.some((m) => m.name === 'docker' || m.name === 'Dockerfile');
    const hasNginx = arch.modules?.some((m) => m.name === 'nginx' || m.name === 'deploy');
    return `# ${name} — 部署流程
...
function generateTestSkill(name, projectPath, arch) {
    const hasTests = arch.modules?.some((m) => m.name === 'test' || m.name === '__tests__' || m.name === 'tests' || m.name === 'spec');
    return `# ${name} — 测试规范
