...
function getModelConfig(tier) { return modelTiers[tier]; }
// ============ Token 估算工具 ============
const TOKEN_RATIO = {
...
export function estimateTokens(text) {
    ...
    if (zhMatches)
        total += zhMatches.length * TOKEN_RATIO.zh;
    // 移除中文后估算英文/代码
...
// ============ 核心调用逻辑 ============
async function callDeepSeek(modelName, messages, opts) {
...
async function callAnthropic(modelName, messages, opts) {
 ...
 */
export async function llmCall(request, fallbackTiers = []) {
...
// ============ 快捷方法 ============
export async function llmCallLight(messages, fallback = true) {
    return llmCall({ tier: 'light', messages }, fallback ? ['medium', 'strong'] : []);
}
export async function llmCallMedium(messages, fallback = true) {
    return llmCall({ tier: 'medium', messages }, fallback ? ['strong'] : []);
}
export async function llmCallStrong(messages) {
...
export function estimateComplexity(text) {
    ...
    if (length < 200 && codeBlocks === 0 && !hasArchitecture)
        return 'simple';
    if (length > 2000 || codeBlocks > 3 || hasArchitecture || hasReview)
        return 'complex';
    return 'medium';
}
/**
 * 根据复杂度自动选择模型层级
 */
export function selectTierByComplexity(complexity) {
    switch (complexity) {
 ...
 * 自动路由：基于任务复杂度选择模型
 */
export async function llmAutoRoute(messages, overrideTier) {
