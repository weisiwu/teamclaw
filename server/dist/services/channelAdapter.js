 ...
 * 飞书消息格式转换
 */
export function adaptFeishuMessage(raw) {
    // 飞书事件消息格式
 ...
 * 微信消息格式转换
 */
export function adaptWechatMessage(raw) {
    return {
        channel: 'wechat',
 ...
 * Web 消息格式转换
 */
export function adaptWebMessage(raw) {
    return {
        channel: 'web',
 ...
 */
export function adaptMessage(raw, channel) {
    switch (channel) {
        case 'feishu':
 ...
 * 从飞书消息中检测角色
 */
function detectRoleFromFeishu(sender) {
    // 飞书特殊成员或管理员标识
 ...
 * 从微信消息中检测角色
 */
function detectRoleFromWechat(raw) {
    // 微信通过 RemarkName 或特定标签判断（此处简化处理）
    const remark = String(raw.RemarkName || '');
    if (remark.includes('[管理员]'))
        return 'admin';
    if (remark.includes('[副管理员]'))
        return 'vice_admin';
    return 'employee';
}
/**
 ...
 * 检测消息中 @ 的 Agent
 */
function detectMentionedAgent(content) {
