 ...
 */
export async function generateFeatureMap(projectPath, projectId, projectName) {
    const features = [];
...
function findFiles(dir, patterns, maxDepth, currentDepth = 0, results = []) {
...
function extractFeatureName(filePath, baseDir) {
...
export async function getFeatureMap(projectId) {
    const outputPath = path.join(process.env.PROJECT_DATA_DIR || '/tmp/teamclaw/projects', projectId, 'feature-map.json');
