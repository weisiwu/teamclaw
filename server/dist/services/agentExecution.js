...
import { llmAutoRoute, estimateComplexity, selectTierByComplexity, } from "./agentExecution.js";
// ============ 内存存储 ============
const executionLogs = new Map();
const agentExecStates = new Map();
// ============ 执行引擎核心 ============
/**
 * 派发任务到指定 Agent（异步执行真实 LLM 调用）
 * 立即返回 executionId，后台异步执行 LLM
 */
export function dispatchToAgent(req) {
 ...
 */
async function executeAgentTask(context, timeoutMs) {
 ...
 * 为 Agent 构建 LLM 消息列表
 */
function buildAgentMessages(agentName, taskId, prompt) {
    // 构建任务上下文
 ...
 * 从 LLM 输出中提取子任务派发指令
 */
async function maybeDispatchSubtasks(context, llmResponse) {
...
export function updateExecution(executionId, updates) {
    ...
    if (!ctx)
        return false;
    if (updates.status)
        ctx.status = updates.status;
    if (updates.result)
        ctx.result = updates.result;
    if (updates.error)
        ctx.error = updates.error;
    if (updates.usage)
        ctx.usage = updates.usage;
    if (updates.costUsd !== undefined)
        ctx.costUsd = updates.costUsd;
 ...
 */
async function onExecutionComplete(ctx) {
    try {
 ...
 * 获取 Agent 的当前执行状态
 */
export function getAgentExecutionState(agentName) {
...
export function getExecutionHistory(opts) {
    ...
    if (opts.agentName)
        items = items.filter((e) => e.targetAgent === opts.agentName);
    if (opts.dispatcher)
        items = items.filter((e) => e.dispatcher === opts.dispatcher);
    if (opts.taskId)
        items = items.filter((e) => e.taskId === opts.taskId);
    if (opts.status)
 ...
 * 强制终止 Agent 的当前执行
 */
export function abortExecution(agentName, reason) {
    const state = agentExecStates.get(agentName);
    if (!state)
        return false;
 ...
 * 获取 Agent 统计信息
 */
export function getAgentExecutionStats(agentName) {
