 ...
 */
import { bumpVersion, bumpLevelFromTaskType, parseSemver } from './semver.js';
/**
 * Determine bump level from task metadata
 */
export function determineBumpLevel(context) {
    return bumpLevelFromTaskType(context.taskType);
}
/**
 * Execute a version bump
 */
export function executeBump(currentVersion, bumpType) {
    return bumpVersion(currentVersion, bumpType);
}
/**
 * Generate a changelog entry from task metadata
 */
export function generateBumpChangelog(context) {
 ...
 */
export function performBump(currentVersion, context) {
 ...
 */
export function formatBumpSummary(result) {
    const lines = [
