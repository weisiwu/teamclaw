...
async function readDirRecursive(dir, maxDepth = 2) {
    const files = [];
...
export async function detectTechStack(projectPath) {
            ...
            for (const [key, val] of Object.entries(deps)) {
                ...
                if (v?.includes('orm') || v?.includes('prisma') || v?.includes('sequi')) {
                    ...
                    if (norm.includes('prisma'))
                        orm.push('Prisma');
                    else if (norm.includes('sequelize'))
                        orm.push('Sequelize');
                    else if (norm.includes('typeorm'))
                        orm.push('TypeORM');
                    else if (norm.includes('drizzle'))
                        orm.push('Drizzle ORM');
                }
            }
        }
        catch {
    ...
    if (present.has('Cargo.toml'))
        runtime.push('Rust');
    if (present.has('requirements.txt') || present.has('pyproject.toml'))
        runtime.push('Python');
    if (present.has('pom.xml') || present.has('build.gradle'))
        runtime.push('JVM');
    // Detect full-stack
    const fullStack = [];
    if (framework.includes('Next.js') || framework.includes('Nuxt.js'))
        fullStack.push('SSR/Isomorphic');
    if (present.has('__init__.py'))
        fullStack.push('Backend API');
    if (present.has('server') || present.has('api'))
        fullStack.push('API Layer');
    return {
