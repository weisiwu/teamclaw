...
function parseGitLog(projectPath, limit = 200) {
    if (!existsSync(join(projectPath, '.git')))
        return [];
    try {
        const output = execSync(`git log --format="%H|%aI|%s|%an" -n ${limit}`, { cwd: projectPath, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
 ...
 */
function categorizePattern(message) {
    const lower = message.toLowerCase();
    if (/fix|bug|修复|错误/.test(lower))
        return '修复Bug';
    if (/style|样式|格式|美化/.test(lower))
        return '修改样式';
    if (/feat|feature|功能|新特性/.test(lower))
        return '新增功能';
    if (/refactor|重构/.test(lower))
        return '代码重构';
    if (/docs?|文档/.test(lower))
        return '文档更新';
    if (/test|测试/.test(lower))
        return '测试相关';
    if (/perf|优化|性能/.test(lower))
        return '性能优化';
    if (/chore|build|ci|部署/.test(lower))
        return '构建/部署';
    if (/security|安全/.test(lower))
        return '安全修复';
    if (/ui|界面|组件/.test(lower))
        return 'UI/组件';
    return '其他改动';
}
/**
 ...
 */
export function analyzeGitHistory(projectPath) {
    ...
    for (const commit of commits) {
        ...
        if (p.sampleCommits.length < 3)
            p.sampleCommits.push(commit.hash);
    }
 ...
 */
export function getGitSummaryForMemory(projectPath) {
    const analysis = analyzeGitHistory(projectPath);
    if (!analysis.totalCommits)
        return '';
