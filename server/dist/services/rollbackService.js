...
import { checkout, getCurrentBranch, createBranch, tagExists } from './gitService.js';
/**
 * Rollback to a specific tag
 */
export function rollbackToTag(projectPath, tagName, options = {}) {
 ...
 */
export function rollbackToBranch(projectPath, branchName, options = {}) {
    if (!existsSync(join(projectPath, '.git'))) {
        return {
            success: false,
            previousRef: '',
 ...
 */
export function rollbackToCommit(projectPath, commitHash, options = {}) {
    if (!existsSync(join(projectPath, '.git'))) {
        return {
            success: false,
            previousRef: '',
 ...
 */
export function getRollbackPreview(projectPath, targetRef) {
    if (!existsSync(join(projectPath, '.git'))) {
        return {
            targetRef,
            currentRef: '',
    ...
    try {
        ...
        const diffOutput = execSync(`git log --oneline ${targetRef}..${currentRef} 2>/dev/null | head -50`, { cwd: projectPath, encoding: 'utf-8', timeout: 10000 });
        ...
        const aheadOutput = execSync(`git log --oneline ${currentRef}..${targetRef} 2>/dev/null | head -50`, { cwd: projectPath, encoding: 'utf-8', timeout: 10000 });
        ...
        let filesChanged = [];
        try {
            const filesOutput = execSync(`git diff --name-only ${targetRef} ${currentRef} 2>/dev/null`, { cwd: projectPath, encoding: 'utf-8', timeout: 10000 });
 ...
 */
export function getRollbackTargets(projectPath, options = {}) {
    if (!existsSync(join(projectPath, '.git'))) {
    ...
    try {
        // Get tags
        const tagsOutput = execSync(`git tag -l --format='%(refname:short)||%(objectname)||%(creatordate:iso)' --sort=-creatordate | head -${maxTags}`, { cwd: projectPath, encoding: 'utf-8', timeout: 10000 });
        const tags = tagsOutput
            ...
            .filter(t => t.name);
        // Get branches
        const branchesOutput = execSync(`git branch -a --format='%(refname:short)||%(HEAD)' | head -${maxBranches}`, { cwd: projectPath, encoding: 'utf-8', timeout: 10000 });
