...
import { semanticDocSearch, applyFilters, getSearchSuggestions, saveSearchHistory, clearSearchHistory, indexDocInChroma, } from '../services/searchEnhancer.js';
...
router.get('/docs', async (req, res) => {
    const { q, mode = 'keyword', type, dateFrom, dateTo, projectId, sizeMin, sizeMax, page = '1', pageSize = '10', userId = 'default' } = req.query;
    const pageNum = parseInt(page);
    const size = parseInt(pageSize);
    const filter = { type: type, dateFrom: dateFrom, dateTo: dateTo, projectId: projectId, sizeMin: sizeMin ? parseInt(sizeMin) : undefined, sizeMax: sizeMax ? parseInt(sizeMax) : undefined };
    // Semantic search via ChromaDB
    if (mode === 'semantic' && q && typeof q === 'string') {
        const semanticResults = await semanticDocSearch(q, size);
        ...
        saveSearchHistory(userId, q, 'semantic', filter, total);
    ...
    let docs = docService.getDocList(q);
    docs = applyFilters(docs, filter);
    const total = docs.length;
    const paged = docs.slice((pageNum - 1) * size, pageNum * size);
    if (q && typeof q === 'string')
        saveSearchHistory(userId, q, 'keyword', filter, total);
    res.json(success({
        list: paged.map(d => ({
            type: 'doc',
            id: d.id,
...
router.get('/suggestions', (req, res) => {
    ...
    const suggestions = getSearchSuggestions(q, parseInt(limit));
...
router.get('/history', (req, res) => {
    ...
    const history = getSearchHistoryRecords(userId, parseInt(limit));
