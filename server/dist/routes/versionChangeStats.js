...
router.get('/', (req, res) => {
    ...
    let row = db.prepare('SELECT * FROM versions WHERE id = ? OR version = ? OR git_tag = ?').get(tag, tag, tag);
    if (!row) {
        // Try with v prefix
        const withV = tag.startsWith('v') ? tag : `v${tag}`;
        row = db.prepare('SELECT * FROM versions WHERE id = ? OR version = ? OR git_tag = ?').get(withV, withV, withV);
    }
    if (!row) {
