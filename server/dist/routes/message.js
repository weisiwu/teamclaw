...
router.patch('/:messageId/status', (req, res) => {
    try {
        ...
        const validStatuses = ['pending', 'processing', 'completed', 'suspended'];
        ...
        if (status === 'completed') {
            ...
            if (msg)
                messageStatsService.onCompleted(msg);
        }
...
router.get('/ratelimit/check', (req, res) => {
    try {
        ...
        const queueStatus = messageQueueService.getQueueStatus();
        const result = messageRateLimiterService.check(String(userId), String(role), String(channel), queueStatus.total);
        res.json(success(result));
    }
    catch (err) {
...
router.put('/ratelimit/config', (req, res) => {
    try {
        ...
        if (!key)
            return res.status(400).json(error(400, '缺少 key 参数'));
