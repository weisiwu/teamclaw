...
import { createPackage, getPackageInfo, deletePackage, listPackages, getPackageFilePath, } from '../services/packageService.js';
...
router.get('/', (req, res) => {
    const { versionId, limit = '20', offset = '0' } = req.query;
    if (!versionId) {
...
router.get('/latest/:versionId', (req, res) => {
...
router.post('/', async (req, res) => {
    const { versionId, versionName, versionNumber, buildCommand, projectPath, triggeredBy = 'system', triggerType = 'manual' } = req.body;
...
router.get('/stats/:versionId', (req, res) => {
...
router.post('/:id/rollback', (req, res) => {
    ...
    const { target, targetType = 'commit', createBranch = false } = req.body;
...
router.post('/:id/package', async (req, res) => {
    ...
    const result = await createPackage(record.versionId, record.buildNumber, projectPath, record.artifactPaths, safeFormat);
    if (!result.success) {
