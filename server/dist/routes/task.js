...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        if (!body.sessionId || !body.title) {
            res.status(400).json(error(400, 'sessionId and title are required'));
            return;
        }
...
router.post('/:taskId/cancel', async (req, res) => {
    try {
        ...
        res.json(success({ taskId, cancelled }));
    }
    catch (err) {
...
router.get('/:taskId/prompt', (req, res) => {
    try {
        ...
        if (!sessionId) {
            res.status(400).json(error(400, 'sessionId required'));
            return;
        }
...
router.get('/sla/:taskId', (req, res) => {
    try {
        ...
        if (!sla)
            return res.status(404).json(error(404, 'No SLA found'));
...
router.get('/sla', (req, res) => {
    try {
        ...
        res.json(success(getAllSLAs(status)));
    }
    catch (err) {
...
router.post('/sla/:taskId/deadline', (req, res) => {
    try {
        ...
        if (!deadline)
            return res.status(400).json(error(400, 'deadline required'));
