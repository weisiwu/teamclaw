...
router.get('/messages', async (req, res) => {
    try {
        ...
        const { container_id_type = 'chat', container_id, start_time, end_time, page_size = '20', page_token, sort_type = 'ByCreateTimeDesc', } = req.query;
        if (!container_id) {
...
router.get('/chats', async (req, res) => {
    try {
        ...
        const response = await fetch(`https://open.feishu.cn/open-apis/im/v1/chats?${queryParams.toString()}`, {
            method: 'GET',
            headers: {
...
async function getAppAccessTokenSimple(appId, appSecret) {
