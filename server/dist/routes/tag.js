...
router.post('/', (req, res) => {
    const { name, versionId, versionName, message, commitHash, createdBy, projectPath: reqProjectPath } = req.body;
...
router.get('/preview/:version', (req, res) => {
...
router.post('/bulk', requireAdmin, (req, res) => {
    const { tagNames, action } = req.body;
    ...
    for (const tagName of tagNames) {
        try {
            ...
            if (action === 'archive') {
                ...
                updateTagRecord(record.id, { archived: false });
            }
