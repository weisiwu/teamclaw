...
import { importArtifactsFromDir, } from '../services/artifactStore.js';
import { getLatestBuildRecord } from '../models/buildRecord.js';
import path from 'path';
import os from 'os';
const router = Router();
// Get project path for a version
function getVersionProjectPath(versionId) {
...
router.get('/:versionId', (req, res) => {
    ...
    if (buildNumber) {
        ...
        record = records.find((r) => r.buildNumber === parseInt(buildNumber));
        if (!record) {
...
router.get('/:versionId/:buildNumber', (req, res) => {
    const { versionId, buildNumber } = req.params;
    const { file } = req.query;
    const records = require('../models/buildRecord').getBuildRecordsByVersion(versionId, 100);
    const record = records.find((r) => r.buildNumber === parseInt(buildNumber));
    if (!record) {
        return res.status(404).json(error(404, 'Build not found'));
    }
...
router.post('/:versionId/:buildNumber/import', async (req, res) => {
    const { versionId, buildNumber } = req.params;
    const records = require('../models/buildRecord').getBuildRecordsByVersion(versionId, 100);
    const record = records.find((r) => r.buildNumber === parseInt(buildNumber));
    if (!record) {
