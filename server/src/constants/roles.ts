...
// 普通员工与 pm 交互时的能力范围
export type MemberCapability = "full" | "assistant_only";

export const MEMBER_PM_CAPABILITIES: Record<Role, MemberCapability> = {
  admin: "full",
  vice_admin: "full",
 ...
 */
export function canAccessAgent(role: Role, agent: AgentName): boolean {
 ...
 * 获取用户与 pm 交互时的能力范围
 */
export function getPmCapability(role: Role): MemberCapability {
 ...
 */
export function compareRoleWeight(roleA: Role, roleB: Role): number {
 ...
 * 获取优先级高于指定角色的最小角色
 */
export function getHigherRole(role: Role): Role | null {
  const sortedRoles = (Object.keys(ROLE_WEIGHTS) as Role[]).sort(
    (a, b) => ROLE_WEIGHTS[b] - ROLE_WEIGHTS[a]
  );
