...
interface StatsByRole {
  admin: { enqueued: number; completed: number; failed: number };
  vice_admin: { enqueued: number; completed: number; failed: number };
  employee: { enqueued: number; completed: number; failed: number };
}

interface StatsByChannel {
  feishu: number;
  wechat: number;
  slack: number;
  web: number;
...
class MessageStatsService {
  private static instance: MessageStatsService;
  ...
  // 按渠道统计
  private byChannel: StatsByChannel = {
    feishu: 0,
    wechat: 0,
    slack: 0,
    web: 0,
  ...
  // 时间序列（用于滑动窗口统计）
  private recentMessages: { msg: Message; event: 'enqueued' | 'completed' | 'failed' }[] = [];
  private readonly MAX_RECENT = 10000;

  // 累计的 top 用户（滚动窗口）
  private topUsersMap: Map<string, { userName: string; count: number }> = new Map();

  static getInstance(): MessageStatsService {
   ...
   * 添加到最近消息序列
   */
  private addRecent(msg: Message, event: 'enqueued' | 'completed' | 'failed'): void {
   ...
   * 计算时间窗口统计
   */
  private calcTimeWindow(windowMs: number): TimeWindowStats {
   ...
   * 获取 top N 用户
   */
  private getTopUsers(n: number = 10): { userId: string; userName: string; count: number }[] {
   ...
   * 获取完整统计
   */
  getStats(queueDepth: number, currentProcessing: string | null): MessageStats {
    return {
