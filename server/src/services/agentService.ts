 ...
 * 获取单个 Agent 详情
 */
export function getAgent(name: string): AgentDetail | null {
...
export function updateAgentConfig(
  ...
  if (idx >= 0) {
    if (updates.defaultModel) AGENT_TEAM[idx].defaultModel = updates.defaultModel;
    if (updates.capabilities) AGENT_TEAM[idx].capabilities = updates.capabilities;
  }
 ...
 */
export function getAgentSessions(name: string): { sessionId: string; updatedAt: string; label: string }[] {
 ...
 * 获取团队概览
 */
export function getTeamOverviewData(): TeamOverview {
 ...
 * 更新 Agent 运行时状态
 */
export function updateAgentStatus(name: string, status: AgentStatus, taskId?: string): boolean {
  const runtime = agentRuntimes.get(name);
 ...
 */
export function getAvailableAgents(level?: number): string[] {
  const agents = level
