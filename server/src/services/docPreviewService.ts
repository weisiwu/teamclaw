...
import { DocPreviewResult, DocPreviewConfig } from '../models/download.js';
...
const PREVIEW_CONFIG: DocPreviewConfig = {
...
function wrapCodeHtml(code: string, language: string): string {
...
export async function generatePreviews(
  files: Array<{ docId: string; filePath: string }>,
  options?: { maxLines?: number }
): Promise<Map<string, DocPreviewResult>> {
  const results = new Map<string, DocPreviewResult>();

  await Promise.all(
    files.map(async ({ docId, filePath }) => {
      try {
...
export function getSupportedPreviewTypes(): string[] {
