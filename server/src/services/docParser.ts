...
function parseMarkdown(filePath: string, stat: fs.Stats): ParsedDocument {
...
function parseText(filePath: string, stat: fs.Stats): ParsedDocument {
  const content = fs.readFileSync(filePath, 'utf-8');
  return {
    path: filePath,
    content,
...
export function extractKeyPhrases(text: string, maxPhrases = 20): string[] {
  // 简单关键词提取：出现频率高的单词
