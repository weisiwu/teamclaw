...
class TaskFlowService {
  private static instance: TaskFlowService;

  private constructor() {}

  static getInstance(): TaskFlowService {
   ...
   * 添加子任务
   */
  addSubtask(parentTaskId: string, subtaskId: string): boolean {
   ...
   */
  canStart(taskId: string): { canStart: boolean; reasons: string[] } {
    const task = taskLifecycle.getTask(taskId);
    if (!task) return { canStart: false, reasons: ['Task not found'] };
   ...
   * 获取可开始的任务列表（按依赖排序）
   */
  getRunnableTasks(sessionId?: string): Task[] {
   ...
   * 完成子任务时，更新父任务进度
   */
  async onSubtaskCompleted(subtaskId: string): Promise<void> {
   ...
   */
  async cascadeCancel(taskId: string): Promise<string[]> {
    const cancelled: string[] = [taskId];
    const task = taskLifecycle.getTask(taskId);
   ...
   * 获取任务链（所有祖先和后代）
   */
  getTaskChain(taskId: string): { ancestors: string[]; descendants: string[] } {
