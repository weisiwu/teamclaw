...
export interface SLADefinition {
  priority: SLAPriority;    // 对应的任务优先级
  warningThreshold: number;  // 剩余多少分钟时触发预警（到截止时间）
  deadlineHours: number;     // 截止时间（小时），从任务创建起算
}

export interface TaskSLA {
  taskId: string;
  deadline: string;         // ISO 8601 绝对截止时间
  slaLevel: SLAPriority;
  status: SLAStatus;
  remainingMinutes: number;   // 剩余分钟数（负数=已超时）
  warningSent: boolean;      // 是否已发送预警
  breachedAt?: string;      // 违约时间
  completedAt?: string;     // 完成时间（用于计算实际耗时）
  exceededByMinutes?: number; // 超出分钟数
}
...
export interface SLAStats {
  ...
  atRisk: number;
  ok: number;
  completionRate: number;    //按时完成率
  avgResponseMinutes: number; // 平均响应时间（分钟）
  breachedTasks: { taskId: string; title: string; exceededByMinutes: number }[];
  atRiskTasks: { taskId: string; title: string; remainingMinutes: number }[];
}

// 默认SLA定义（可配置）
const DEFAULT_SLA: SLADefinition[] = [
...
// 内存存储：任务SLA映射
const taskSLAs = new Map<string, TaskSLA>();
const slaDefinitions: SLADefinition[] = [...DEFAULT_SLA];

// ============ SLA 初始化 ============

export function initSLAForTask(taskId: string): TaskSLA | null {
  const task = taskStore.get(taskId);
  if (!task) return null;

  // 如果已有SLA，直接返回
  if (taskSLAs.has(taskId)) return taskSLAs.get(taskId)!;
...
function updateSLAStatus(sla: TaskSLA, taskStatus: string): void {
  const now = new Date();
...
export function getTaskSLA(taskId: string): TaskSLA | undefined {
  const sla = taskSLAs.get(taskId);
  if (!sla) return initSLAForTask(taskId);
  // 实时更新状态
  const task = taskStore.get(taskId);
  if (task) updateSLAStatus(sla, task.status);
  return sla;
}

export function getAllSLAs(status?: SLAStatus): TaskSLA[] {
  const allSLAs: TaskSLA[] = [];
...
export function getBreachedSLAs(): TaskSLA[] {
  return getAllSLAs('breached');
}

export function getAtRiskSLAs(): TaskSLA[] {
  return getAllSLAs('at_risk');
}

// ============ SLA 预警检查 ============

export function checkSLAWarnings(): TaskSLA[] {
  const warned: TaskSLA[] = [];
  for (const [taskId, sla] of taskSLAs.entries()) {
    const task = taskStore.get(taskId);
    if (!task || task.status !== 'pending' && task.status !== 'running') continue;
    if (sla.warningSent) continue;
...
export function getSLAStats(timeWindowHours = 24 * 7): SLAStats {
  ...
  const breachedTasks: SLAStats['breachedTasks'] = [];
  const atRiskTasks: SLAStats['atRiskTasks'] = [];
  ...
  for (const task of tasks) {
    ...
    total++;
    switch (sla.status) {
      case 'breached': breached++; breachedTasks.push({ taskId: task.taskId, title: task.title, exceededByMinutes: sla.exceededByMinutes ?? 0 }); break;
      case 'at_risk': atRisk++; atRiskTasks.push({ taskId: task.taskId, title: task.title, remainingMinutes: sla.remainingMinutes }); break;
  ...
  const avgResponseMinutes = completedSLAs.length > 0
    ? Math.round(completedSLAs.reduce((sum, s) => {
        ...
        if (!task || !s.completedAt) return sum;
...
export function setTaskDeadline(taskId: string, deadlineISO: string): boolean {
  const task = taskStore.get(taskId);
  if (!task) return false;

  const slaDef = slaDefinitions.find(s => s.priority === task.priority) ?? slaDefinitions[2];
  const sla: TaskSLA = taskSLAs.get(taskId) ??
...
export function updateSLADefinitions(defs: SLADefinition[]): void {
