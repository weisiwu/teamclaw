...
// 内存存储：活跃任务列表
const activeTasks: Map<string, DispatchRequest> = new Map();
 ...
 * 根据优先级计算负载增量
 */
function getPriorityLoadIncrement(priority: string): number {
  switch (priority) {
 ...
 * 获取活跃任务列表
 */
export function getActiveTasks(): DispatchRequest[] {
 ...
 * 获取指定 Agent 的当前任务
 */
export function getAgentCurrentTask(agentName: string): DispatchRequest | null {
  for (const task of activeTasks.values()) {
    if (task.toAgent === agentName) return task;
  }
  return null;
}
