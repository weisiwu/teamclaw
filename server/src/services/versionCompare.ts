...
export interface VersionCompareResult {
  ...
  changelogs: {
    from: VersionChangelog | null;
    to: VersionChangelog | null;
  };
  summary: CompareSummary;
}

export interface CompareSummary {
  newerIsAhead: boolean;  // to is newer than from
  commitDelta: number;    // to - from
  fileDelta: number;      // to - from
 ...
 */
function getProjectPath(versionId: string): string {
  return path.join(
 ...
 */
export async function getVersionCommits(
  versionId: string,
  ref?: string,
  maxCount = 50
): Promise<CommitInfo[]> {
 ...
 */
function compareCommits(fromCommits: CommitInfo[], toCommits: CommitInfo[]): VersionCompareResult['commits'] {
 ...
 */
function compareFiles(fromCommits: CommitInfo[], toCommits: CommitInfo[]): VersionCompareResult['files'] {
...
function generateSummary(
  fromVersion: string,
  toVersion: string,
  result: Pick<VersionCompareResult, 'commits' | 'files' | 'changelogs'>
): CompareSummary {
...
export async function quickCompare(
  ...
  fromTag: string,
  toTag: string
): Promise<{ hasDiff: boolean; summary: string }> {
  try {
