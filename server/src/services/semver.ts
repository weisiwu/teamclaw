 ...
 */
export function formatSemver(parsed: ParsedSemver, includeV = true): string {
 ...
 */
export function versionDistance(from: string, to: string): VersionDiff | null {
  ...
  if (b.major !== a.major) {
    level = 'major';
    // Count major bumps
    for (let m = a.major; m < b.major; m++) distance++;
    return { level, distance, from, to };
  }

  if (b.minor !== a.minor) {
    level = 'minor';
    for (let m = a.minor; m < b.minor; m++) distance++;
    return { level, distance, from, to };
  }
 ...
 */
export function bumpVersion(current: string, type: SemverLevel): string | null {
  const parsed = parseSemver(current);
  if (!parsed) return null;

  const bumped: ParsedSemver = { ...parsed };
...
/**
 * Validate a version bump type based on task metadata
 * Maps task type labels to semver bump levels
 */
export type TaskType = 'feature' | 'fix' | 'improvement' | 'refactor' | 'breaking' | 'docs' | 'hotfix';

const TASK_TYPE_BUMP_MAP: Record<string, SemverLevel> = {
 ...
 */
export function bumpLevelFromTaskType(taskType: string): SemverLevel {
