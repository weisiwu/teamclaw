...
export type ArtifactType = 'js' | 'css' | 'html' | 'image' | 'font' | 'binary' | 'other';
...
function getProjectType(projectPath: string): 'nextjs' | 'node' | 'react' | 'unknown' {
...
function getBuildCommand(projectPath: string): string | null {
  const packageJsonPath = join(projectPath, 'package.json');
...
function collectArtifacts(projectPath: string): BuildArtifact[] {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (stats.size < 100 && artifactType === 'other') continue;
...
export async function runBuild(
  ...
  try {
    ...
    const e = err as { killed?: boolean; code?: number; stdout?: string; stderr?: string };
    const duration = Date.now() - startTime;
 ...
 */
export function getBuildConfig(projectPath: string): {
