...
async function executeMigration(migration: MigrationFile): Promise<void> {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO _migrations (name, checksum, execution_time_ms, status) VALUES ($1, $2, $3, $4) ON CONFLICT (name) DO UPDATE SET status = EXCLUDED.status, execution_time_ms = EXCLUDED.execution_time_ms',
