...
export function runMigrations() {
  ...
  // BranchService SQLite persistence (iter76 - iter-49)
  db.exec(`
  ...
  `);
  try {
    db.prepare("INSERT OR IGNORE INTO branch_config (id, default_branch) VALUES (1, 'main')").run();
  } catch {
    // ignore
  }
  ...
  try {
    db.prepare("ALTER TABLE rollback_history ADD COLUMN actor_id TEXT").run();
  } catch {
    // Column may already exist, ignore
  }
  try {
    db.prepare("ALTER TABLE rollback_history ADD COLUMN metadata TEXT").run();
  } catch {
    // Column may already exist, ignore
  }
