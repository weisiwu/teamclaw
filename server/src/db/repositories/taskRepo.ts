...
export const taskRepo = {
  async upsert(task: {
    ...
    if (existing) {
      ...
      if (task.title !== undefined) { sets.push(`title = $${idx++}`); vals.push(task.title); }
      if (task.description !== undefined) { sets.push(`description = $${idx++}`); vals.push(task.description); }
      if (task.priority !== undefined) { sets.push(`priority = $${idx++}`); vals.push(task.priority); }
      if (task.assignedAgent !== undefined) { sets.push(`assigned_agent = $${idx++}`); vals.push(task.assignedAgent); }
      if (task.startedAt !== undefined) { sets.push(`started_at = $${idx++}`); vals.push(new Date(task.startedAt)); }
      if (task.completedAt !== undefined) { sets.push(`completed_at = $${idx++}`); vals.push(new Date(task.completedAt)); }
      if (task.progress !== undefined) { sets.push(`progress = $${idx++}`); vals.push(task.progress); }
      if (task.lastHeartbeat !== undefined) { sets.push(`last_heartbeat = $${idx++}`); vals.push(new Date(task.lastHeartbeat)); }
      if (task.retryCount !== undefined) { sets.push(`retry_count = $${idx++}`); vals.push(task.retryCount); }
      if (task.result !== undefined) { sets.push(`result = $${idx++}`); vals.push(task.result); }
  ...
  async findBySession(sessionId: string): Promise<TaskRow[]> {
    return query<TaskRow>('SELECT * FROM tasks WHERE session_id = $1 ORDER BY created_at DESC', [sessionId]);
  },

  async findByStatus(status: string): Promise<TaskRow[]> {
    return query<TaskRow>('SELECT * FROM tasks WHERE status = $1 ORDER BY created_at DESC', [status]);
  },
