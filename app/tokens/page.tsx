...
function MainTabs({ active, onChange }: { active: MainTab; onChange: (t: MainTab) => void }) {
  ...
  return (
    <div className="overflow-x-auto scrollbar-hide flex gap-1 border-b border-border mb-6 -mx-4 px-4">
      {tabs.map((t) => (
        <button
          key={t.key}
...
function Tabs({ active, onChange }: { active: Tab; onChange: (t: Tab) => void }) {
  ...
  return (
    <div className="overflow-x-auto scrollbar-hide flex gap-1 border-b border-border -mx-4 px-4">
      {tabs.map((t) => (
        <button
          key={t.key}
          onClick={() => onChange(t.key)}
